...
const WebUIConfigCard = () =>
{
	...
	// 2FA 相关状态
	const [twoFAStatus, setTwoFAStatus] = useState<{ enable2FA: boolean; hasSecret: boolean }>({
		enable2FA: false,
		hasSecret: false
	});
	...
	return (
		<>
			...
			<div className='flex flex-col gap-4'>
				<div className='flex-shrink-0 w-full font-bold text-default-600 dark:text-default-400 px-1'>修改密码
				</div>
				<Controller
					control={pwdControl}
					name='oldToken'
			...
			<div className='flex flex-col gap-2'>
				<div
					className='flex-shrink-0 w-full font-bold text-default-600 dark:text-default-400 px-1'>Passkey认证
				</div>

				{!window.isSecureContext
					? (
						<div
							className='text-sm text-warning-500 bg-warning-50 p-2 rounded-md border border-warning-200'>
			...
			<div className='flex flex-col gap-2'>
				<div
					className='flex-shrink-0 w-full font-bold text-default-600 dark:text-default-400 px-1 flex items-center gap-2'>
					<span>双因素认证 (2FA)</span>
					<span
						className={`text-sm px-2 py-0.5 rounded-full ${twoFAStatus.enable2FA ? 'bg-green-100 text-green-700' : 'bg-default-200 text-default-600'}`}>
					...
					<>
						...
						<div className='bg-default-50 border border-default-200 rounded-lg p-4'>
							...
							{twoFASecret && (
								<>
									<div className='mb-4'>
										<div className='text-sm text-default-600 mb-2'>请使用 Authenticator
											应用扫描下方二维码
										</div>
										<div className='flex items-center gap-4'>
											...
											<div>
												...
												<div
													className='font-mono text-sm bg-default-100 px-3 py-2 rounded break-all max-w-[200px]'>
													{twoFASecret}
												</div>
											</div>
										</div>
