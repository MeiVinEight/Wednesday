...
export default class PluginManager
{
	...
	{
		const {data} = await serverRequest.get<ServerResponse<PluginListResponse>>('/api/v1/Plugin/List');
		return data.data;
	}
	...
	{
		const {data} = await serverRequest.post<ServerResponse<{ message: string; }>>('/api/v1/Plugin/RegisterManager');
		return data.data;
	}
	...
	{
		await serverRequest.post<ServerResponse<void>>('/api/v1/Plugin/SetStatus', {id, enable});
	}
	...
	{
		await serverRequest.post<ServerResponse<void>>('/api/v1/Plugin/Uninstall', {id, cleanData});
	}
	 ...
	 */
	public static async importLocalPlugin(file: File): Promise<{
		message: string;
		pluginId: string;
		installPath: string;
	}>
	...
	public static async getPluginStoreList(forceRefresh: boolean = false): Promise<PluginStoreList>
	{
		...
		const {data} = await serverRequest.get<ServerResponse<PluginStoreList>>('/api/v1/Plugin/Store/List', {params});
		return data.data;
	}
	...
	{
		const {data} = await serverRequest.get<ServerResponse<PluginStoreItem>>(`/api/v1/Plugin/Store/Detail/${id}`);
		return data.data;
	}
	...
	{
		const {data} = await serverRequest.get<ServerResponse<PluginConfigResponse>>('/api/v1/Plugin/Config', {
			params: {id},
		});
		return data.data;
	}
	...
	{
		await serverRequest.post<ServerResponse<void>>('/api/v1/Plugin/Config', {id, config});
	}
	...
	{
		await serverRequest.post<ServerResponse<void>>('/api/v1/Plugin/Config/Change', {
			id,
			sessionId,
			key,
			value,
	...
	public static getConfigSSEUrl(id: string, config?: Record<string, unknown>): string
	{
		...
		return `/api/v1/Plugin/Config/SSE?${params.toString()}`;
	}
}
