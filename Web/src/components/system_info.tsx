...
}> = ({currentVersion, latestVersion, status, errorMessage}) =>
{
	return (
		<div className='space-y-6'>
			{/* 版本对比 */}
			<div
				className='flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-8 bg-default-50 dark:bg-default-100/5 rounded-xl border border-default-100 dark:border-default-100/10'>
				<div className='flex flex-col items-center gap-2 min-w-0 w-full sm:w-auto'>
					...
					<Tooltip content={`v${currentVersion}`}>
						<Chip size='md' variant='flat' color='default'
						      classNames={{content: 'font-mono font-bold text-sm truncate max-w-[120px] sm:max-w-[160px]'}}>
							v{currentVersion}
						</Chip>
					</Tooltip>
				</div>
				...
				<div className='flex flex-col items-center text-primary-500 px-4 shrink-0'>
					<div className='p-2 rounded-full bg-primary-50 dark:bg-primary-900/20'>
						<svg className='w-6 h-6 animate-pulse rotate-90 sm:rotate-0' fill='none' viewBox='0 0 24 24'
						     stroke='currentColor'>
			...
			{status === 'updating' && (
				<div
					className='flex flex-col items-center justify-center gap-3 py-4 px-4 rounded-lg bg-primary-50/50 dark:bg-primary-900/20 border border-primary-200/50 dark:border-primary-700/30'>
			...
			{status === 'success' && (
				<div
					className='flex flex-col items-center justify-center gap-3 py-4 px-4 rounded-lg bg-success-50/50 dark:bg-success-900/20 border border-success-200/50 dark:border-success-700/30'>
					<div
						className='w-12 h-12 rounded-full bg-success-100 dark:bg-success-900/40 flex items-center justify-center'>
						<svg className='w-6 h-6 text-success-600 dark:text-success-400' fill='none' viewBox='0 0 24 24'
						     stroke='currentColor'>
					...
					<div
						className='mt-2 p-3 rounded-lg bg-warning-50/50 dark:bg-warning-900/20 border border-warning-200/50 dark:border-warning-700/30'>
			...
			{status === 'error' && (
				<div
					className='flex flex-col items-center justify-center gap-3 py-4 px-4 rounded-lg bg-danger-50/50 dark:bg-danger-900/20 border border-danger-200/50 dark:border-danger-700/30'>
					<div
						className='w-12 h-12 rounded-full bg-danger-100 dark:bg-danger-900/40 flex items-center justify-center'>
						<svg className='w-6 h-6 text-danger-600 dark:text-danger-400' fill='none' viewBox='0 0 24 24'
						     stroke='currentColor'>
							<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2}
	...
	return (
		<Tooltip content='有新版本可用'>
			<div className='cursor-pointer flex items-center justify-center'
			     onClick={updateStatus === 'updating' ? undefined : showUpdateDialog}>
				<Chip
					size='sm'
					color='primary'
					variant='flat'
				...
				>
					{updateStatus === 'updating' ?
						<Spinner size='sm' color='primary' classNames={{wrapper: 'w-3 h-3'}}/> : 'New'}
				</Chip>
			</div>
		</Tooltip>
	);
	...
	return (
		<div className='space-y-4'>
                            ...
                            <span className='truncate max-w-[300px]'>
                              {version.type === 'action'
								  ...
								  : version.tag}
													...
													{version.type === 'action' && (
														<div
															className='text-xs text-default-400 flex items-center gap-2'>
															<span
																className='font-mono bg-default-100 dark:bg-default-100/10 px-1 rounded'>{version.tag}</span>
															{version.headSha && <span className='font-mono'
															                          title={version.headSha}>{version.headSha.slice(0, 7)}</span>}
															{version.createdAt &&
			...
			{selectedVersion && isSelectedDowngrade && (
				<div
					className='p-3 rounded-lg bg-warning-50/50 dark:bg-warning-900/20 border border-warning-200/50 dark:border-warning-700/30'>
					<div className='flex items-start gap-2'>
						<svg className='w-5 h-5 text-warning-600 dark:text-warning-400 flex-shrink-0 mt-0.5' fill='none'
