														 ...
														 }) =>
{
	...
	const AuthorComponent = (
		<span
			className={clsx('font-medium transition-colors', homepage ? 'hover:text-primary hover:underline cursor-pointer' : '')}>
      {author || '未知作者'}
    </span>
	);
						...
						<Tooltip
							content={
								<div className='flex flex-wrap gap-1 max-w-[200px] p-1'>
									{tags.map(t => (
										<span key={t}
										      className='text-xs bg-white/10 px-1.5 py-0.5 rounded-md border border-white/10'>
                      {t}
                    </span>
									))}
								</div>
					...
					: installStatus === 'update-available'
						...
						: (
							<Button
								...
								startContent={<MdOutlineGetApp size={20}
