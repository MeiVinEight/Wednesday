	...
	return (
		...
		>
			...
			<CardHeader className='pb-0 flex flex-col items-start gap-1'>
				...
				<div
					className='text-xs text-primary-200 whitespace-nowrap overflow-hidden text-ellipsis w-full'>{theme.description}</div>
			</CardHeader>
			<CardBody>
...
const ThemeConfigCard = () =>
{
	...
	useEffect(() =>
	{
		...
		{
			styleTagRef.current.innerHTML = generateTheme(theme);
		}
	}, [theme]);
					...
					>
						<div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
							{(['light', 'dark'] as const).map((mode) => (
								<Card key={mode}
								      className={clsx('border shadow-sm', mode === 'dark' ? 'bg-[#18181b] border-zinc-800' : 'bg-white border-zinc-200')}>
									<CardHeader className='pb-0 pt-4 px-4 flex-col items-start'>
										<div className='flex items-center gap-2 mb-1'>
											{mode === 'dark' ? <MdDarkMode className='text-zinc-400' size={20}/> :
												<MdLightMode className='text-orange-400' size={20}/>}
                            ...
                            <span
                              ...
                              {colorKey.replace('--heroui-', '')}
														...
														<span
															className={clsx('text-[10px] truncate', mode === 'dark' ? 'text-zinc-500' : 'text-zinc-400')}>
                              Variable
                            </span>
													</div>
												</div>
