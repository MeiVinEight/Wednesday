					...
					: (
						<div className='flex items-center gap-2'>
							<Chip size='sm' color='default' variant='flat'
							      className='text-default-500'>WebUI {WEBUI_VER}</Chip>
							...
							<div className='space-y-3'>
								...
								{password && (
									<div className='space-y-1'>
										{accounts.length === 1 &&
											<p className='text-xs text-default-500'>{currentAccount.label}</p>}
										<Tooltip content={`使用 ${currentAccount.label} 申请入群，以此值作为入群密码`}
										         placement='top'>
											<Snippet
												size='sm'
												variant='flat'
...
export default function AboutPage()
{
	...
	return (
		<div className='flex flex-col h-full w-full gap-6 p-2 md:p-6'>
							...
							<div className='flex flex-wrap gap-2'>
								{['TypeScript', 'React', 'Vite', 'Node.js', 'Electron', 'HeroUI'].map((tech) => (
									<Chip key={tech} size='sm' variant='flat'
									      className='bg-default-100/50 text-default-600'>
										{tech}
									</Chip>
								))}
							</div>
			...
			{/* 底部版权 - 移出 grid 布局 */}
			<div
				className='w-full text-center text-tiny text-default-400 py-4 mt-auto flex flex-col items-center gap-1'>
