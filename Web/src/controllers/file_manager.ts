...
export default class FileManager
{
	public static async listFiles(path: string = '/')
	{
		const {data} = await serverRequest.get<ServerResponse<FileInfo[]>>(
			`/api/v1/File/list?path=${encodeURIComponent(path)}`
		);
		return data.data;
	}
	...
	{
		const {data} = await serverRequest.get<ServerResponse<FileInfo[]>>(
			`/api/v1/File/list?path=${encodeURIComponent(path)}&onlyDirectory=true`
		);
		return data.data;
	}
	...
	public static async readFile(path: string)
	{
		const {data} = await serverRequest.get<ServerResponse<string>>(
			`/api/v1/File/read?path=${encodeURIComponent(path)}`
		);
		return data.data;
	}
	...
	{
		const downloadUrl = `/api/v1/File/download?path=${encodeURIComponent(path)}`;
		toast
			.promise(
				serverRequest
	...
	{
		const downloadUrl = '/api/v1/File/batchDownload';
		toast
			.promise(
				serverRequest
					.post(
	...
	public static async upload(path: string, files: File[])
	{
		...
		const {data} = await serverRequest.post<ServerResponse<boolean>>(
			`/api/v1/File/upload?path=${encodeURIComponent(path)}`,
			formData,
			{
				headers: {
