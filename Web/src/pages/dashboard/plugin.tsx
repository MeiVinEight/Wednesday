			...
			dialog.confirm({
				...
				content: (
					<div className='flex flex-col gap-2'>
						<p className='text-base text-default-800'>确定要卸载插件「<span
							className='font-semibold text-danger'>{plugin.name}</span>」吗? 此操作不可恢复。</p>
						<div className='mt-2 bg-default-100 dark:bg-default-50/10 p-3 rounded-lg flex flex-col gap-1'>
							<label className='flex items-center gap-2 cursor-pointer w-fit'>
								<input
									...
									onChange={(e) =>
									{
										cleanData = e.target.checked;
									}}
									className='w-4 h-4 cursor-pointer accent-danger'
								/>
								<span className='text-small font-medium text-default-700'>同时删除其配置文件</span>
							</label>
							<p className='text-xs text-default-500 pl-6 break-all w-full'>配置目录:
								config/plugins/{plugin.id}</p>
						</div>
					</div>
				),
				confirmText: '确定卸载',
			...
			<div className='p-2 md:p-4 relative'>
				...
				{pluginManagerNotFound
					...
					: plugins.length === 0
						...
						: (
							<div
								className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 justify-start items-stretch gap-x-2 gap-y-4'>
								{plugins.map(plugin => (
									<PluginDisplayCard
										key={plugin.id}
										data={plugin}
