		...
		<>
			...
			<div className='p-2 md:p-4 relative h-[calc(100vh-6rem)] md:h-[calc(100vh-4rem)] flex flex-col'>
				...
				{extensionPages.length === 0 && !loading
					...
					: (
						<div
							className='flex-1 min-h-0 bg-white/40 dark:bg-black/20 backdrop-blur-md rounded-lg overflow-hidden relative'>
							{iframeLoading && (
								<div
									className='absolute inset-0 flex items-center justify-center bg-default-100/50 z-10'>
									<Spinner size='lg'/>
								</div>
							)}
							<iframe
