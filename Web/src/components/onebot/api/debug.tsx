			...
			>
				...
				<div className='flex items-center gap-2'>
					<Popover placement='bottom-end' backdrop='transparent'>
						...
						<PopoverContent
							className='w-[260px] p-3 rounded-md border border-white/10 shadow-2xl bg-white/80 dark:bg-black/80 backdrop-blur-xl'>
							<div className='flex flex-col gap-2'>
								<p className='text-[10px] font-bold opacity-30 uppercase tracking-widest'>Debug
									Setup</p>
								<Input label='Base URL' labelPlacement='outside' placeholder='http://...'
								       value={httpConfig.url}
								       onChange={(e) => setHttpConfig({...httpConfig, url: e.target.value})} size='sm'
								       variant='bordered'/>
								<Input label='Token' labelPlacement='outside' placeholder='access_token'
								       value={httpConfig.token}
								       onChange={(e) => setHttpConfig({...httpConfig, token: e.target.value})} size='sm'
								       variant='bordered'/>
							</div>
						</PopoverContent>
					</Popover>
							...
							<ChatInputModal>
								{(onOpen) => (
									<Tooltip content='构造 CQ 码' closeDelay={0}>
										<Button isIconOnly size='sm' variant='light' radius='sm' className='w-8 h-8'
										        onPress={onOpen}>
						...
						{activeTab === 'request'
							...
							: (
								<div className='p-6 space-y-8 overflow-y-auto h-full scrollbar-hide'>
									<section>
										<h3 className='text-[10px] font-bold text-default-700 dark:text-default-50 uppercase tracking-widest mb-4'>Request
											Params</h3>
										<DisplayStruct schema={parsedRequest}/>
									</section>
									<div className='h-px bg-white/10 w-full'/>
									<section>
										<h3 className='text-[10px] font-bold text-default-700 dark:text-default-50 uppercase tracking-widest mb-4'>Response
											Data</h3>
					...
					<div className='flex items-center gap-2'>
						...
						{responseStatus && (
							<Chip size='sm' variant='dot'
							      color={responseStatus.code >= 200 && responseStatus.code < 300 ? 'success' : 'danger'}
							      className='h-5 text-[10px] font-mono border-none bg-transparent pl-0'>
								{responseStatus.code} {responseStatus.text}
							</Chip>
						)}
					</div>

					<Button size='sm' variant='light' isIconOnly radius='sm'
					        className='h-6 w-6 opacity-40 hover:opacity-100' onClick={(e) =>
					{
						e.stopPropagation();
