	...
	const renderField = (item: PluginConfigSchemaItem) =>
	{
		...
		switch (item.type)
		{
			...
			case 'boolean':
				return (
					<div key={item.key}
					     className='flex justify-between items-center mb-4 p-2 bg-default-100 rounded-lg'>
			...
			case 'html':
				return (
					<div key={item.key} className='mb-4'>
						...
						<div dangerouslySetInnerHTML={{__html: item.default || ''}}
					...
					<>
						<ModalHeader className='flex flex-col gap-1'>
							<div className='flex items-center gap-2'>
								...
								{supportReactive && (
									<span
										className={`text-tiny px-2 py-0.5 rounded ${connected ? 'bg-success-100 text-success-600' : 'bg-warning-100 text-warning-600'}`}>
                    {connected ? '已连接' : '未连接'}
                  </span>
								)}
							</div>
							...
							{loading
								...
								: (
									<div className='flex flex-col gap-2'>
										{schema.length === 0
											? (
												<div className='text-center text-default-500'>No configuration schema
													available.</div>
											)
											: (
